...
def get_courses():  # access the DB and get all the courses and puts them in a list
    ...
    for result in results:
        print(result[1])
        courses_list.append(str(result[1]))
...
class SearchCourse(UserControl):
    ...
    def get_description(self, value):
        ...
        cursor.execute(f"SELECT course_descrip FROM class WHERE course_num=?", [value])
    ...
    def confirm(self, e):
        ...
        try:
            if self.courses.controls[1].controls[0].leading.value == True:
                ...
                cursor = connection.cursor()
                cursor.execute(
                    "INSERT INTO user_class(user_ID, dept_ID, course_num) VALUES (:user, :dept_ID, :course_num)",
                    {'user': userID, 'dept_ID': "COSC", 'course_num': f"{your_course.course_name}"}
                )
                connection.commit()
                connection.close()
